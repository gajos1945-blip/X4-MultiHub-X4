...
def test_rc_build_manifest_contract():
    ...
    assert '"version": "0.9-rc1"' in source
    assert '"release_status": "RELEASE_CANDIDATE_HARDWARE_UNVERIFIED"' in source
...
def test_rc_workflow_and_filename():
    ...
    assert "BUILD X4 MULTIHUB RC BIN" in workflow
    assert "X4_MultiHub_X4_v0_9_rc1" in workflow
    assert "X4_MultiHub_X4_v0.9-rc1.bin" in workflow
...
def test_rc_settings_version_string():
    ...
    assert "X4 MultiHub 0.9-rc1" in source
